...
class HaxeGotoBuildFieldsCommand( sublime_plugin.TextCommand ):
    def run( self , edit ) :
        ...
        function_list = [[p.to_string() + " - " + p.kind, p.type.file] for k in filtered_types for p in filtered_types[k].all_fields_list]
        function_list_data = [(k + "." + p.name,p) for k in filtered_types for p in filtered_types[k].all_fields_list]
        ...
        def on_selected (i):
            ...
            if i >= 0:
                ...
                if run_on_select == "goto":
                    ...
                    _find_decl_file = selected_type[1].type.file
                    ...
                    def show():
                        win.open_file(selected_type[1].type.file)
                    sublime.set_timeout(show, 130)
                else:
                    viewtools.insert_at_cursor(view, selected_type[1].type.full)
...
class HaxeGotoBuildFieldsListener(sublime_plugin.EventListener):
